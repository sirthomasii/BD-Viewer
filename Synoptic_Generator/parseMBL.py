...
class MBL():
    def __init__(self):
        import yaml
        self.dict=yaml.load(open(os.path.join(os.path.dirname(sys.argv[0]),'dictionary.yml'),'r').read())['MBL']
    ...
    def append(self, element, essname, insightLink, model, section, aperture, index, slot_number, slot_type, doorsid, elementEnergy, TCSz, TCSy, MCSz, MCSy):
        ...
        if element in ('QV', 'QH', 'CX', 'BPM', 'BCM', 'BLM', 'WS', 'NPM', 'LBM', 'IBS'):
            if model:
            ...
            if index == 1:
                ...
                with self.tag('div', klass="element BLE", index=index, essname="n/a", insightLink="n/a", model="n/a", slot_type=slot_type, slot_number=slot_number, aperture=aperture, elementEnergy=elementEnergy, section=section, tcs_z=TCSz, tcs_y=TCSy, mcs_y=MCSy, mcs_z=MCSz, id=self.dict['Valve']['DN100']['id']):
                  #text('index = '+index)
                   self.doc.stag('img', klass="element_img", src=self.dict['Valve']['DN100']['src'])
                ...
                with self.tag('div', klass="element BLE", index=index, essname=essname, insightLink=insightLink, model=model, slot_type=slot_type, slot_number=slot_number, aperture=aperture, elementEnergy=elementEnergy, section=section, tcs_z=current_CM_TCS_z_middle, tcs_y=TCSy, mcs_y=MCSy, mcs_z=current_CM_MCS_z_middle, id=element):
                   #text('index = '+index)
            ...
            print("Ignoring element",element,"in MBL")
