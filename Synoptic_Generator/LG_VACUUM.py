...
if len(sys.argv)>1:
    ...
    ELEMENTS_CSV_VACUUM=sys.argv[3]
else:
    ...
    ELEMENTS_CSV_VACUUM=os.path.join(folder, 'TARGET', 'vacuum2.csv')
...
#quit()
#ADD VACUUM STUFF
with open(ELEMENTS_CSV_VACUUM)  as csvfile:
     ...
     for h,r in zip(headers,res):
         ...
         content[h]=r
# print(content['sect'])
vacuum_index = list(content['index'])
vacuum_index = [v.lstrip('0') for v in vacuum_index]
vacuum_index = list(map(int, vacuum_index[:-1]))

numTotalElements = len(index)
numVacuumElements = len(list(content['elem']))
# print(numVacuumElements)

vacuum_TCSz = list(map(float, content['TCS,m,z[mm]']))

#for every vacuum element
for i in range(0, numVacuumElements):
    #search for that TCS point that is less than your value
    #print(i)
    for j in range(0, numTotalElements):
        #if you find the the TCS point that is lower
        if((vacuum_TCSz[i]<float(TCSz[j])) and (vacuum_TCSz[i]>float(TCSz[j-1])) ):
            #insert the row!
            section.insert(j, content['sect'][i])
            essname.insert(j, content['essname'][i])
            model.insert(j, content['model'][i])
            slot_number.insert(j, content['cell'][i])
            slot_type.insert(j, content['slot'][i])
            element.insert(j, content['elem'][i])
            index.insert(j, content['index'][i])
            doorsid.insert(j, content['doorsid'][i])
            aperture.insert(j, content['r[mm]'][i])
            TCSz.insert(j, content['TCS,m,z[mm]'][i])
            TCSy.insert(j, content['TCS,m,y[mm]'][i])
            MCSz.insert(j, content['MCS,m,z[mm]'][i])
            MCSy.insert(j, content['MCS,m,y[mm]'][i])
